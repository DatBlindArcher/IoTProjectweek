...
function setup_environment {
  ...
  SUPPORT="Leuven: support.leuven@ucll.be, Limburg: support.limburg@ucll.be"
if [ ! -z "$DISPLAY" ] ; then
...
if ! ask "This installer will only work properly if you are a member of ${bf}KU Leuven Association${n} and the user group: ${bf}UC Leuven-Limburg.${n}" "Continue" 1 ; then exit; fi
...
class EduroamNMConfigTool:
    ...
    def add_connection(self,ssid):
        server_alt_subject_name_list = dbus.Array({'DNS:idp-rad1.khleuven.be','DNS:idp-rad2.khleuven.be','DNS:radius.ucll.be'})
        server_name = 'be'
        if self.nm_version == "0.9" or self.nm_version == "1.0":
    ...
    def main(self):
        ...
        self.delete_existing_connections('eduroam5G')
        self.add_connection('eduroam5G')
